...
DEFAULT_DB_OPTS = {'admin':'postgres', #FIXME: Should be rhaptos_dba
                   'adminpass':None,
                   'user':'rhaptos',
                   'userpass':None,
                   'dbname':'repository-newplone25-2', #FIXME: Should be repository
                   'server':None,
                   'port':5433, #FIXME: Should be None
                   }

import logging
logger = logging.getLogger('RhaptosModuleStorage.Install')
...
def install(self):
    ...
    """
    out = StringIO()
    log("Starting Install of RhaptosModuleStorage", out)
    ...
    if not dbtool or not dbtool.db:
        ...
        log("...setting up DB connection", out)
        setupDBConnection(self, portal)
        d = portal._dbopts

        # Add and configure the tool
        if not dbtool:
            log("...adding ModuleDB Tool", out)
            portal.manage_addProduct['RhaptosModuleStorage'].manage_addTool('Module DB Tool')
            dbtool = getToolByName(self, 'portal_moduledb')
        dbtool.setDB(d['dbname']+'DA')

        log(installdb(self), out)
    else:
    ...
    if not STORAGE in portal.content.listStorages():
        log("...registering storage: %s" % STORAGE, out)
    ...
    #typestool = getToolByName(self, 'portal_types')
        ...
        #log("...registering dummy type: %s" % TYPE, out)
    ...
    # Setup permission
    portal.manage_permission(RATE_MODULE_PERMISSION, ('Member',), acquire=1)

    log("Completed Install of RhaptosModuleStorage", out)
    return out.getvalue() 
